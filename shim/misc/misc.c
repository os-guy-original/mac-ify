 ...
 *
 * We provide a minimal kqueue emulation backed by epoll. This is enough
 * for Go's runtime to initialize. For network I/O, we'd need full kqueue
 * semantics, but for simple commands like `rclone version`, this stub
 * works.
 *
...
/* ── Go binary stubs ─────────────────────────────────────────
 * Go binaries reference many macOS-specific functions that don't exist
 * on Linux. We provide no-op stubs for the ones that Go's runtime
...
/* Mach VM stubs */
int mach_vm_region(uint64_t target_task, uint64_t *address, uint64_t *size,
