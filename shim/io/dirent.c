...
DIR *macify_opendir(const char *name) {
    ...
    if (!real_opendir) real_opendir = macify_elf_lookup("opendir");
...
int macify_readdir_r(DIR *dirp, struct macos_dirent *entry, struct macos_dirent **result) {
    if (!real_readdir_r) real_readdir_r = macify_elf_lookup("readdir_r");
    struct dirent le;
...
static struct macos_dirent *do_readdir_impl(DIR *dirp) {
    ...
    if (!real_readdir) real_readdir = macify_elf_lookup("readdir");
